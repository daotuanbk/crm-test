...
import { RolesService, roleRepository } from '@app/auth';
...
const roleService: RolesService = {
  setup: (app, path) => {
    ...
    app.get(path + '/get-salesman', async (req: any, res: Response) => {
      try {
        const salesmanRole = await roleRepository.findSalesman();
  ...
  findSalesman: async () => {
    return await roleRepository.findSalesman();
  },
  findInterviewer: async () => {
    return await roleRepository.findInterviewer();
  },
  find: async (params) => {
    // 1. authorize
