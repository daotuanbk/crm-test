...
import { PROSPECTING_STAGE_STATUS } from '@client/core';
...
export const ContactStatusScreen = Authorize(class extends React.Component<Props, State> {
  state: State = {
    search: '',
    emailConfigs: [],
    stages: [],
  ...
  async componentDidMount() {
    ...
    const stages = await serviceProxy.findSystemConfigs('getAllContactStages', undefined, undefined, undefined, undefined, undefined, undefined) as any;
    if (stages) {
      this.setState({
        stages: stages.data,
      });
  ...
  handleSearch = async (params: any) => {
    ...
    try {
      const idToken = await firebase.auth().currentUser!.getIdToken();
      const serviceProxy = getServiceProxy(idToken);
      const defaultFilter = 'option|' + PROSPECTING_STAGE_STATUS;
    ...
    try {
      ...
      if (this.state.modal.create) {
        result = await serviceProxy.createSystemConfig({
          option: PROSPECTING_STAGE_STATUS,
          value: {
            ...values,
          },
        });
  ...
  render() {
    return (
      <div>
        ...
        <Row>
          <Col xs={12}>
            <h2 style={{ marginBottom: 24 }}>{translate('lang:allContactStatuses')}</h2>
          </Col>
          <Col xs={12} style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <Button type='primary' onClick={() => this.openCentreModal()}>{translate('lang:addContactStatus')}</Button>
          </Col>
        </Row>
        {(this.state.modal.create || this.state.modal.update) && (
          <ContactStatusModal
            title={this.state.modal.update ? translate('lang:updateContactStatus') : translate('lang:createContactStatus')}
