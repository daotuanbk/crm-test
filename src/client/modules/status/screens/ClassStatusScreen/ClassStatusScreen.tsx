...
import './ClassStatusScreen.less';
import { Authorize, BorderWrapper, Pagination } from '@client/components';
import { message, Row, Col, Button } from 'antd';
import { Main } from './Main';
import { PERMISSIONS } from '@common/permissions';
import { LEAD_CLASS_STAGE_STATUS } from '@client/core';
import { ClassStatusModal } from './ClassStatusModal';
...
export const ClassStatusScreen = Authorize(class extends React.Component<Props, State> {
  state: State = {
    search: '',
    emailConfigs: [],
    stages: [],
  ...
  async componentDidMount() {
    ...
    const stages = await serviceProxy.findSystemConfigs('getAllClassStages', undefined, undefined, undefined, undefined, undefined, undefined) as any;
    if (stages) {
      this.setState({
        stages: stages.data,
      });
  ...
  handleSearch = async (params: any) => {
    ...
    try {
      const idToken = await firebase.auth().currentUser!.getIdToken();
      const serviceProxy = getServiceProxy(idToken);
      const defaultFilter = 'option|' + LEAD_CLASS_STAGE_STATUS;
    ...
    try {
      ...
      if (this.state.modal.create) {
        result = await serviceProxy.createSystemConfig({
          option: LEAD_CLASS_STAGE_STATUS,
          value: {
            ...values,
          },
        });
  ...
  render() {
    return (
      <div>
        <Row>
          <Col xs={12}>
            <h2 style={{ marginBottom: 24 }}>{translate('lang:allClassStatuses')}</h2>
          </Col>
          <Col xs={12} style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <Button type='primary' onClick={() => this.openCentreModal()}>{translate('lang:addClassStatus')}</Button>
          </Col>
        </Row>
        {(this.state.modal.create || this.state.modal.update) && (
          <ClassStatusModal
            title={this.state.modal.update ? translate('lang:updateClassStatus') : translate('lang:createClassStatus')}
