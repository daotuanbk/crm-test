...
import './ClassStageScreen.less';
import { Authorize, BorderWrapper, Pagination } from '@client/components';
import { message, Row, Col, Button } from 'antd';
import { Main } from './Main';
import { LEAD_CLASS_STAGE } from '@client/core';
import { PERMISSIONS } from '@common/permissions';
import { ClassStageModal } from './ClassStageModal';
...
export const ClassStageScreen = Authorize(class extends React.Component<Props, State> {
  state: State = {
    search: '',
    emailConfigs: [],
    data: [],
  ...
  handleSearch = async (params: any) => {
    ...
    try {
      const idToken = await firebase.auth().currentUser!.getIdToken();
      const serviceProxy = getServiceProxy(idToken);
      const defaultFilter = 'option|' + LEAD_CLASS_STAGE;
  ...
  render() {
    return (
      <div>
        <Row>
          <Col xs={12}>
            <h2 style={{ marginBottom: 24 }}>{translate('lang:allClassStages')}</h2>
          </Col>
          <Col xs={12} style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <Button type='primary' onClick={() => this.openCentreModal()}>{translate('lang:addClassStage')}</Button>
          </Col>
        </Row>
        {(this.state.modal.create || this.state.modal.update) && (
          <ClassStageModal
            title={this.state.modal.update ? translate('lang:updateClassStage') : translate('lang:createClassStage')}
