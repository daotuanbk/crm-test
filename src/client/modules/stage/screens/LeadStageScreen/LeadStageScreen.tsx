...
export const LeadStageScreen = Authorize(class extends React.Component<Props, State> {
  state: State = {
    search: '',
    emailConfigs: [],
    data: [],
  ...
  handleSearch = async (params: any) => {
    ...
    try {
      const idToken = await firebase.auth().currentUser!.getIdToken();
      const serviceProxy = getServiceProxy(idToken);
      const defaultFilter = 'option|' + LEAD_STAGE;
  ...
  render() {
    return (
      <div>
        <Row>
          <Col xs={12}>
            <h2 style={{ marginBottom: 24 }}>{translate('lang:allLeadStages')}</h2>
          </Col>
          <Col xs={12} style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <Button type='primary' onClick={() => this.openCentreModal()}>{translate('lang:addLeadStage')}</Button>
          </Col>
        </Row>
        {(this.state.modal.create || this.state.modal.update) && (
          <LeadStageModal
            title={this.state.modal.update ? translate('lang:updateLeadStage') : translate('lang:createLeadStage')}
